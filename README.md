...
VOLUME /tmp
EXPOSE 8765
ADD ./target/hr-gateway-0.0.1-SNAPSHOT.jar hr-gateway.jar
ENTRYPOINT ["java","-jar","/hr-gateway.jar"]
``` 
```
mvnw clean package -DskipTests

docker build -t hr-gateway:v1 .

docker run -p 8765:8765 --name hr-gateway --network hr-net hr-gateway:v1
```
